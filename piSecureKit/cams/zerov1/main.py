...
video_cfg = picam2.create_video_configuration(
    ...
    buffer_count=3,     # try 2 if you still see CMA issues
)
picam2.align_configuration(video_cfg)
picam2.configure(video_cfg)

enc = H264Encoder(bitrate=2_000_000, repeat=True, intra_period=60)  # repeat=insert SPS/PPS

# Ask ffmpeg to generate clean PTS for RTSP and push over TCP
rtsp_url = f"rtsp://{USER}:{PASS}@{HUB}:8554/lqstream"
ff = (
    "-fflags +genpts -use_wallclock_as_timestamps 1 "
    "-rtsp_transport tcp -muxdelay 0 -muxpreload 0 "
    "-c:v copy -an -f rtsp " + rtsp_url
)
out = FfmpegOutput(ff)

print("Starting camera stream…")
picam2.start_recording(enc, out)
picam2.wait()
